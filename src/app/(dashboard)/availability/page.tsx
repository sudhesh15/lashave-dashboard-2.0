...
function AvailabilityContent() {
  ...
  const [openBookingFilter, setOpenBookingFilter] = useState<"status" | "date" | null>(null);
  const [bookingDateRange, setBookingDateRange] = useState<{ from: string; to: string } | null>(null);
  const [bookingDatePreset, setBookingDatePreset] = useState<number | null>(null);
  const bookingFilterRef = useRef<HTMLDivElement>(null);
  const bookingDateFilterRef = useRef<HTMLDivElement>(null);
  useOutsideClick(bookingFilterRef, () => setOpenBookingFilter(null), openBookingFilter === "status");
  useOutsideClick(bookingDateFilterRef, () => setOpenBookingFilter(null), openBookingFilter === "date");
  ...
  const filteredBookings = useMemo(() => {
    ...
    const byDate = bookingDateRange
      ? byTab.filter((booking) => {
          const key = (booking.start_time || booking.booking_date || "").slice(0, 10);
          if (!key) return false;
  ...
  const sortedBookings = useMemo(() => {
    const getSortValue = (booking: Booking) => {
      ...
      switch (bookingSort.key) {
        case "created":
          return booking.created_at || booking.booking_date || booking.start_time || "";
        case "customer":
  ...
  const primaryCustomerFields = primaryFieldKeys
    .map((key) =>
      customerFields.find(
        (field) => field.field_key.toLowerCase() === key,
      ),
    )
  ...
  const customCustomerFields = customerFields.filter(
    (field) =>
      !primaryFieldKeys.includes(field.field_key.toLowerCase()),
  );
        ...
        <div className="min-w-0 px-5 py-5 sm:px-6">
          <div className="flex flex-col gap-4 rounded-t-xl border border-b-0 border-gray-200 bg-white px-5 py-4 dark:border-white/[0.05] dark:bg-white/[0.01] lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
              ...
              <div ref={bookingFilterRef} className="relative">
                <Button
                  variant="outline"
                  onClick={() => setOpenBookingFilter(openBookingFilter === "status" ? null : "status")}
                >
                  <SlidersHorizontal size={14} />
                  Filter
                </Button>
              ...
              <div ref={bookingDateFilterRef}>
                <DateFilter
                  ...
                  onToggle={() => setOpenBookingFilter(openBookingFilter === "date" ? null : "date")}
          ...
          <div className="min-w-0 max-w-full overflow-hidden rounded-b-xl border border-gray-200 dark:border-white/[0.05]">
            <div className="w-full overflow-x-auto">
              <table className="lashvae-column-dividers min-w-[1560px] table-fixed">
                <colgroup>
                    ...
                    {[
                      ...
                      <th
                        ...
                        className={cn(
                          ...
                          ["phone", "email", "channel", "date", "time"].includes(column.key) && "px-6",
                  ...
                  {paginatedBookings.length === 0 ? (
                    ...
                    paginatedBookings.map((booking) => {
                      ...
                      const contactPhone =
                        ...
                        booking.customer_phone ||
                        "";
                      const contactEmail = booking.customer_details?.email || "";
                      const effectiveStatus = isBookingCompleted(booking)
                        ? "completed"
                        : booking.status;
        ...
        <div className="w-full rounded-3xl bg-white p-6 dark:bg-gray-900">
          ...
          <div className="mt-6 flex justify-end gap-3">
            <Button
              ...
              disabled={!!bookingToComplete && actionLoading === bookingToComplete.id}
            >
              Keep Booking
            </Button>
            <Button
